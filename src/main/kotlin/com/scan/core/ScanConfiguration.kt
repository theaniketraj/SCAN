...
/**
 * Main configuration class for the SCAN plugin Supports hierarchical configuration loading and
 * validation
 */
data class ScanConfiguration(
    ...
    companion object {
        ...
        val DEFAULT_INCLUDED_EXTENSIONS =
                setOf(
                        // Source code
                        "kt",
                        "kts",
    ...
    fun mergeWith(other: ScanConfiguration): ScanConfiguration {
        return copy(
                ...
                includePatterns =
                        if (other.includePatterns != DEFAULT_INCLUDE_PATTERNS) other.includePatterns
                        else this.includePatterns,
                excludePatterns = this.excludePatterns + other.excludePatterns,
                includedExtensions =
                        if (other.includedExtensions != DEFAULT_INCLUDED_EXTENSIONS)
                                other.includedExtensions
                        else this.includedExtensions,
                excludedExtensions = this.excludedExtensions + other.excludedExtensions,
                maxFileSize =
                        if (other.maxFileSize != DEFAULT_MAX_FILE_SIZE) other.maxFileSize
                        else this.maxFileSize,
...
data class DetectorConfiguration(
    ...
    fun validate(): List<ConfigurationError> {
        ...
        customDetectors.forEach { detector -> errors.addAll(detector.validate()) }
    ...
    fun mergeWith(other: DetectorConfiguration): DetectorConfiguration {
        return copy(
                enabledDetectors =
                        if (other.enabledDetectors != setOf("pattern", "entropy", "context"))
                                other.enabledDetectors
                        else this.enabledDetectors,
...
data class PatternConfiguration(
    ...
    fun validate(): List<ConfigurationError> {
        ...
        customPatterns.forEach { pattern -> errors.addAll(pattern.validate()) }
    ...
    fun mergeWith(other: PatternConfiguration): PatternConfiguration {
        return copy(
                ...
                patternSets =
                        if (other.patternSets != setOf("default", "api-keys", "crypto", "database"))
                                other.patternSets
                        else this.patternSets,
...
data class EntropyConfiguration(
    ...
    fun validate(): List<ConfigurationError> {
        ...
        if (charset !in setOf("base64", "hex", "alphanumeric", "ascii")) {
            errors.add(
                    ConfigurationError("charset must be one of: base64, hex, alphanumeric, ascii")
            )
        }
...
data class FilterConfiguration(
        ...
        val generatedFileMarkers: List<String> =
                listOf(
                        "// Generated by",
                        "/* Generated by",
                        "# Generated by",
    ...
    fun mergeWith(other: FilterConfiguration): FilterConfiguration {
        return copy(
                enabledFilters =
                        if (other.enabledFilters != setOf("extension", "path", "whitelist", "test"))
                ...
                generatedFileMarkers =
                        if (other.generatedFileMarkers != this.generatedFileMarkers)
                                other.generatedFileMarkers
                        else this.generatedFileMarkers
        )
    }
}
...
data class ReportingConfiguration(
    ...
    fun mergeWith(other: ReportingConfiguration): ReportingConfiguration {
        return copy(
                enabled = other.enabled,
                formats =
                        if (other.formats != setOf(ReportFormat.CONSOLE)) other.formats
                        else this.formats,
                outputPath =
                        if (other.outputPath != "scan-results") other.outputPath
...
data class PerformanceConfiguration(
    ...
    fun mergeWith(other: PerformanceConfiguration): PerformanceConfiguration {
        return copy(
                maxConcurrency =
                        if (other.maxConcurrency != Runtime.getRuntime().availableProcessors())
                                other.maxConcurrency
                        else this.maxConcurrency,
                timeoutSeconds =
                        if (other.timeoutSeconds != 300) other.timeoutSeconds
                        else this.timeoutSeconds,
                memoryLimitMB =
                        if (other.memoryLimitMB != 1024) other.memoryLimitMB
                        else this.memoryLimitMB,
                ...
                cacheDirectory =
                        if (other.cacheDirectory != ".scan-cache") other.cacheDirectory
...
data class ValidationConfiguration(
    ...
    fun mergeWith(other: ValidationConfiguration): ValidationConfiguration {
        return copy(
                ...
                timeoutSeconds =
                        if (other.timeoutSeconds != 10) other.timeoutSeconds
                        else this.timeoutSeconds,
                maxConcurrentValidations =
                        if (other.maxConcurrentValidations != 5) other.maxConcurrentValidations
                        else this.maxConcurrentValidations,
                cacheValidationResults = other.cacheValidationResults,
                cacheTtlHours =
                        if (other.cacheTtlHours != 24) other.cacheTtlHours else this.cacheTtlHours,
...
class ScanConfigurationBuilder {
    ...
    fun includePatterns(patterns: List<String>) = apply {
        config = config.copy(includePatterns = patterns)
    }
    fun excludePatterns(patterns: List<String>) = apply {
        config = config.copy(excludePatterns = patterns)
    }
    fun includedExtensions(extensions: Set<String>) = apply {
        config = config.copy(includedExtensions = extensions)
    }
    fun excludedExtensions(extensions: Set<String>) = apply {
        config = config.copy(excludedExtensions = extensions)
    }
    ...
    fun detectors(detectors: DetectorConfiguration) = apply {
        config = config.copy(detectors = detectors)
    }
    fun patterns(patterns: PatternConfiguration) = apply {
        config = config.copy(patterns = patterns)
    }
    fun entropy(entropy: EntropyConfiguration) = apply { config = config.copy(entropy = entropy) }
    fun contextAnalysis(context: ContextConfiguration) = apply {
        config = config.copy(contextAnalysis = context)
    }
    fun filters(filters: FilterConfiguration) = apply { config = config.copy(filters = filters) }
    fun whitelist(whitelist: WhitelistConfiguration) = apply {
        config = config.copy(whitelist = whitelist)
    }
    fun reporting(reporting: ReportingConfiguration) = apply {
        config = config.copy(reporting = reporting)
    }
    fun performance(performance: PerformanceConfiguration) = apply {
        config = config.copy(performance = performance)
    }
    fun validation(validation: ValidationConfiguration) = apply {
        config = config.copy(validation = validation)
    }
    fun buildIntegration(buildIntegration: BuildIntegrationConfiguration) = apply {
        config = config.copy(buildIntegration = buildIntegration)
    }
    ...
    fun from(existing: ScanConfiguration) = apply { config = existing }
    fun from(other: ScanConfigurationBuilder) = apply { config = other.build() }
